...
def minimize(fa: FA) -> FA:
    ...
    """
    print("For Minimization, we transform the FA in CDFA without non-accessible states if it was not already the case :")
    import copy
    ...
    print("the groups (of the partition 0) are : ")
...
recognized from one state and not recognized from the other. """
        ...
        transition_f = FA(len(initials), len(terminals), initials, terminals, n_nodes)
        display(transition_f)

        i += 1
        print(f"instance n°{i}")
        print(f"the groups (of the partition {i}) are : ")
        for k in new_groups.keys():
            print('- group ' + str(k) + ": ", end="")
            for elt in new_groups[k]:
                print(elt, end=", ")
    ...
    print("the groups (of the final partition) are : ")
    for k in new_groups.keys():
        print('- group ' + str(k) + ": ", end="")
        for elt in new_groups[k]:
            print(elt, end=", ")
